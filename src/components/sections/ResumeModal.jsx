...
const ResumeModal = ({ isOpen, onClose }) => {
  ...
  // Lock background document body scroll when modal is active to prevent double scrollbars
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Clean up printing class after print dialog finishes
  useEffect(() => {
...
ITM University | Score: 78%`;
  ...
  const handleBackdropClick = (e) => {
    if (e.target.id === 'modal-backdrop-overlay' || e.target.id === 'modal-scroll-wrapper') {
      sound.playLensClick();
      onClose();
    }
  };
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-black/90 backdrop-blur-2xl animate-fadeIn custom-scrollbar select-none"
      role="dialog"
      aria-modal="true"
      aria-labelledby="resume-modal-title"
    >
      {/* Scrollable Page Wrapper: The page itself scrolls smoothly */}
      <div
        id="modal-scroll-wrapper"
        onClick={handleBackdropClick}
        className="min-h-screen w-full flex flex-col items-center py-5 sm:py-8 md:py-10 px-3 sm:px-6"
      >
        {/* Optimized Floating Sticky Action Controls Bar */}
        <header className="sticky top-3 sm:top-4 z-40 w-full max-w-[210mm] mb-5 sm:mb-7 px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl bg-zinc-950/90 backdrop-blur-xl border border-white/15 shadow-[0_15px_40px_rgba(0,0,0,0.6)] flex items-center justify-between gap-2 sm:gap-3 no-print select-none">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 flex-shrink">
            ...
            <div className="flex items-center gap-1.5 font-mono text-xs text-white truncate">
              ...
              <span className="text-cyan-400 font-semibold tracking-wider text-[11px] sm:text-xs">RESUME</span>
            </div>
          </div>

          {/* Center & Right: High-Utility Segmented Button Console */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            {/* Modal Close Button: Integrated seamlessly inside container */}
            <button
              type="button"
              onClick={() => {
                sound.playClick();
        ...
        {/* A4 Size Resume Sheet: Exactly 210mm x 297mm Ratio, No Inner Scroll Container */}
        <div
          className={`w-full max-w-[210mm] min-h-[297mm] p-4 sm:p-10 md:p-12 transition-colors duration-300 ${
            isDarkMode
...
ITM University | Score: 78%`}
            ...
            <>
              ...
              <div className="resume-section-block">
                <h2 className={`font-mono text-xs uppercase tracking-widest font-bold mb-2 flex items-center gap-2 ${
              ...
              <div className="resume-section-block">
                <h2 className={`font-mono text-xs uppercase tracking-widest font-bold mb-2.5 flex items-center gap-2 ${
                  isDarkMode ? 'text-cyan-400' : 'text-sky-800'
                }`}>
              ...
              {/* Bottom Quick Actions Footer Strip */}
              <div className={`mt-8 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-4 font-mono text-xs no-print ${
                isDarkMode ? 'border-white/10 text-zinc-400' : 'border-zinc-200 text-zinc-600'
              }`}>
                <div className="flex items-center gap-2 text-[11px]">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                  <span>Available for Full-time Roles, Brand Retainers & Strategic Directing.</span>
                </div>
                ...
                <div className="flex items-center gap-2.5 flex-wrap justify-center sm:justify-end">
                  ...
                  <button
                    ...
                    onClick={handleCopyEmail}
                    data-cursor="COPY"
                    className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border text-xs transition-all duration-200 ${
                      isDarkMode
                  ...
                  <a
                    ...
                    data-cursor="DOWNLOAD"
                    onClick={() => sound.playClick()}
                    className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-xs shadow-[0_0_15px_rgba(56,189,248,0.3)] transition-all duration-200 active:scale-95"
                  >
                    <Download className="w-3.5 h-3.5 stroke-[2.5]" />
